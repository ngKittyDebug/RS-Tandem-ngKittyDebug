...
export const GameCards: Card[] = [
  ...
  },
  {
    ...
    cardDescriptionEn:
      'NaN (short for Not-a-Number, "not a number") in JavaScript is a special value of the "number" type, which indicates the result of an incorrect or indefinite mathematical operation. typeof NaN will return "number", although technically it is "not a number".',
    cardDescriptionRu:
      'NaN (сокращение от Not-a-Number, «не число») в JavaScript — это специальное значение типа «число» (number), которое обозначает результат некорректной или неопределенной математической операции. typeof NaN вернет "number", хотя технически это «не число».',
  },
  {
    id: '12',
    cardName: 'console',
    ...
    cardDescriptionEn:
      "A console in JavaScript is an embedded object that provides access to the browser's debugging console (or environment Node.js ). It allows developers to output messages, check the values of variables, and track errors in the code.",
    cardDescriptionRu:
      'Console (консоль) в JavaScript — это встроенный объект, предоставляющий доступ к отладочной консоли браузера (или среды Node.js). Она позволяет разработчикам выводить сообщения, проверять значения переменных и отслеживать ошибки в коде.',
  },
  {
    id: '13',
    cardName: 'RegExp',
    ...
    cardDescriptionEn:
      'Regular expressions (RegExp) in JavaScript is a powerful tool for searching, replacing, and verifying substrings in strings based on the use of special templates. They are an object that defines a search pattern used to match sequences of characters.',
    cardDescriptionRu:
      'Регулярные выражения (RegExp) в JavaScript — это мощный инструмент для поиска, замены и проверки подстрок в строках, основанный на использовании специальных шаблонов. Они представляют собой объект, который определяет шаблон поиска, используемый для сопоставления последовательностей символов.',
  },
  {
    id: '14',
    cardName: 'Set',
    ...
    cardDescriptionEn:
      'Set objects are a collection of values. A value in a Set can occur only once; it is unique in the collection. You can iterate through the set elements in the insertion order. The insertion order corresponds to the order in which each item was successfully inserted into the collection by the add() method (that is, when add() was called, there was no such item in the set yet).',
    cardDescriptionRu:
  ...
  },
  {
    ...
    cardDescriptionEn:
      'Undefined is a data type in JavaScript. This is a primitive value that is assigned to a variable declared without initializing or assigning any value to it. By default, the variable is saved with the Undefined value.',
    cardDescriptionRu:
      'Undefined — это тип данных в JavaScript. Это примитивное значение, которое присваивается переменной, объявленной без инициализации или присвоения ей какого-либо значения. По умолчанию переменная сохраняется со значением Undefined.',
  },
  {
    id: '16',
    cardName: 'null',
  ...
  },
  {
    ...
    cardDescriptionEn:
      'JavaScript Promises make handling asynchronous operations like API calls, file loading, or time delays easier. Think of a Promise as a placeholder for a value that will be available in the future. It can be in one of three states: pending (The task is in the initial state), fulfilled (The task was completed successfully, and the result is available), rejected (The task failed, and an error is provided)',
    cardDescriptionRu:
  ...
  },
  {
    ...
    cardDescriptionEn:
      'The Temporary Dead Zone (TDZ, temporary dead zone) is the period of time from the beginning of the scope (block of code) to the moment when a variable is declared using let or const. At this time, the variable already exists, but accessing it causes a ReferenceError error.',
    cardDescriptionRu:
      'Temporal Dead Zone (TDZ, временная мертвая зона) — это период времени от начала области видимости (блока кода) до момента объявления переменной с помощью let или const. В это время переменная уже существует, но обращение к ней вызывает ошибку ReferenceError.',
  },
  {
    id: '20',
    cardName: 'Hoisting',
  ...
  },
  {
    ...
    cardDescriptionEn:
      'The localStorage property allows you to access the Storage object. localStorage is similar to the sessionStorage property. The only difference is that the sessionStorage property stores data during the session (until the browser is closed), unlike the data in the localStorage property, which has no storage time limits and can only be deleted using JavaScript. Keys and values are always strings (just like objects, integer keys will automatically be converted to strings).',
    cardDescriptionRu:
  ...
  },
  {
    ...
    cardDescriptionEn:
      'Recursion in JavaScript is a programming technique in which a function calls itself inside its body. It is used to break down a complex task into simpler, similar subtasks.',
    cardDescriptionRu:
      'Рекурсия в JavaScript — это приём программирования, при котором функция вызывает саму себя внутри своего тела. Она используется для разбиения сложной задачи на более простые, аналогичные подзадачи.',
  },
  {
    id: '30',
    cardName: 'symbol',
    ...
    cardDescriptionEn:
      'A symbol is a primitive data type used to create unique identifiers. Symbols are created by calling the Symbol() function, to which you can pass the description (name) of the symbol. Even if the characters have the same name, they are different characters.',
    cardDescriptionRu:
      'Символ (symbol) – примитивный тип данных, использующийся для создания уникальных идентификаторов. Символы создаются вызовом функции Symbol(), в которую можно передать описание (имя) символа. Даже если символы имеют одно и то же имя, это – разные символы. ',
  },
];
